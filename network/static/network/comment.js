...
function commentListeners() {
  ...
  let input_comment = document.querySelectorAll(".comment-wrapper form input");
  input_comment.forEach((elem) => {
    if (elem) {
  ...
  del_btn.forEach((btn) => {
    if (btn) btn.addEventListener("click", delComment);
  });
}
...
function delComment(event) {
  ...
  let comment = document.querySelector(
    `.comment-item-${post_id}-${comment_id}`
  );
  // console.log(comment);
  ...
  fetch(`/comments/del/${post_id}/${comment_id}`, {
    ...
    .then((data) => {
      ...
      document.querySelector(`.comment-counter-${post_id}`).innerHTML =
        data["count"];
    })
    .catch((error) => {
      console.log(error);
